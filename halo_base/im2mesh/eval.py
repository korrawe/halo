...
from im2mesh.common import compute_iou, compute_iou_viz

from scripts.visualize_utils import display_iou
...
class MeshEvaluator(object):
    ...
    def eval_mesh(self, mesh, pointcloud_tgt, normals_tgt,
        ...
        if len(mesh.vertices) != 0 and len(mesh.faces) != 0:
            ...
            # print("points_iou", points_iou)
            # print("occ", occ)
            # print("occ", occ.shape)
            # print("occ_tgt", occ_tgt)
            # print("occ_tgt", occ_tgt.shape)
            # out_dict['iou'], union, intersect = compute_iou_viz(occ, occ_tgt)
            # print("union", union)
            # print("intersect", intersect)
            # print('iou', out_dict['iou'])
            # display_iou(points_iou, intersect, union)
