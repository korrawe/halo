...
parser.add_argument('--root_folder', type=str,
                    default='/home/korrawe/nasa/data/youtube_hand/seq_for_vid_4/test',
...
parser.add_argument('--mesh_folder', type=str,
                    default='/home/korrawe/nasa/data/youtube_hand/seq_for_vid_4/test/mesh',
...
parser.add_argument('--meta_folder', type=str,
                    default='/home/korrawe/nasa/data/youtube_hand/seq_for_vid_4/test/meta',
...
def main(args):
    ...
    pose_list = []
    shape_list = []
    # list of selected frames (wrong order)
    # selected_frames = [2,5,10,26,35,14,15]
    # selected_frames = [7,22,11,29,55,64,60]
    # selected_frames = [8,26,100,104,107,88,77]
    selected_frames = [9,27,101,105,108,89,78]
    ...
    for ii, pose in enumerate(pose_list):
        if ii == len(pose_list) - 1 :
            break
        start_pose = pose_list[ii]
        end_pose = pose_list[ii+1]
        start_shape = shape_list[ii]
        end_shape = shape_list[ii+1]
        for j in range(transition_size):

            i = ii * transition_size + j
            print(i)

            interpolated_pose = start_pose + ((end_pose - start_pose) * j) / transition_size
            interpolated_shape = start_shape + ((end_shape - start_shape) * j) / transition_size
            # interpolated_rot = start_rot + ((end_rot - start_rot) * j) / transition_size

            pose_para = torch.cat([rot, interpolated_pose], 1)
            # print(pose_para)
            # assert False
            if not args.fixshape:
